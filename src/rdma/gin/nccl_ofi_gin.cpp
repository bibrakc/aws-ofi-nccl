...
int nccl_ofi_rdma_gin_put_comm::await_pending_requests()
{
	...
	/* Drain any retained one-tail-per-rail tails before waiting below.
	   ...
	   draining here guarantees no retained request is ever left un-rung so
	   the wait cannot hang. */
	{
...
/* Flush the PRIOR retained tail on `rail`. Clear the slot BEFORE posting so a
   hard-error cleanup path never sees a stale tail pointer, and post through
   post_tail_request which records any hard error on the tail's ORIGINAL
   umbrella (not the current op) -- so we only warn here. Caller holds ep_lock. */
void nccl_ofi_rdma_gin_put_comm::flush_tail(uint16_t rail, bool fi_more)
{
	nccl_net_ofi_gin_write_req_t *wreq = tail[rail];
	if (wreq == nullptr) {
		return;
	}
	tail[rail] = nullptr;
	int rc = post_tail_request(wreq, rail, fi_more);
	if (OFI_UNLIKELY(rc != 0 && rc != -FI_EAGAIN)) {
		/* Already recorded on the owning umbrella's status; the current
		   op is unaffected. */
		NCCL_OFI_WARN("GIN tail: prior retained-tail flush failed (%d)", rc);
	}
}

/* Ring every retained tail except `except_rail` (no FI_MORE). Caller holds
   ep_lock. */
void nccl_ofi_rdma_gin_put_comm::flush_other_tails(uint16_t except_rail)
{
	auto &gin_ep = resources.get_ep();
	for (uint16_t r = 0; r < gin_ep.get_num_rails(); r++) {
		if (r != except_rail) {
			flush_tail(r, /*fi_more=*/false);
		}
	}
}

/* Drain every retained tail with real no-FI_MORE posts so no retained request
   is ever left un-rung at close/quiesce. Caller holds ep_lock. */
				  ...
				  nccl_ofi_gin_req_t **request)
{
	...
	if (OFI_LIKELY(size > 0)) {
		...
		if (num_xfers == 1) {
			xfers[0].rail_id = next_rail;
			next_rail = static_cast<uint16_t>((next_rail + 1) %
							  gin_ep.get_num_rails());
		}
		...
		if (num_xfers == 1 && !has_signal) {
			/* Ordinary single-stripe payload put: build the real request
			   but do not post it yet. Depending on the doorbell interval it
			   either becomes this rail's new retained tail or terminates
			   the interval now. */
			...
			/* A newer real request on this rail proves an older same-rail
			   tail is not last, so ring it WITH FI_MORE first. */
			flush_tail(rail, /*fi_more=*/true);

			const bool boundary =
				!aggregate ||
				(requests_since_doorbell + 1 >= GIN_REQS_PER_DOORBELL);
			if (!boundary) {
				/* Retain the current request as this rail's new tail. Its
				   umbrella pending flag and status back-pointer stay live so
				   a later flush attributes any hard error to this owner. */
				tail[rail] = wreq;
			...
			/* Boundary: ring every other rail's tail, then terminate this
			   rail with the current request (no FI_MORE). */
			flush_other_tails(rail);
			requests_since_doorbell = 0;
			int cur_err = post_tail_request(wreq, rail, /*fi_more=*/false);
			nccl_net_ofi_release_schedule(scheduler, schedule);
			if (OFI_UNLIKELY(cur_err != 0 && cur_err != -FI_EAGAIN)) {
				/* post_tail_request already cleared this write's
				   back-pointers and returned it to the pool. */
				resources.return_req_to_pool(req);
				return cur_err;
			}
			rank_comm.tx_head = gin_cursor_inc(rank_comm.tx_head);
		...
		/* EVERY multi-stripe is a hard doorbell boundary: preserve the
		   scheduler's placement, ring retained tails on touched rails with
		   FI_MORE and on untouched rails without, reset the interval, then
		   post the current stripes. Current stripes are never retained. */
		...
		for (uint16_t i = 0; i < num_xfers; i++) {
			...
			/* A put-with-signal posts its first stripe with FI_MORE because
			   the metadata SEND follows on that rail. */
		...
		/* Signal-only is a hard boundary: RR-select the metadata rail, ring
		   that rail's retained tail with FI_MORE (the metadata SEND that
		   follows rings it) and all other rails' tails with no FI_MORE. */
		rail_id = next_rail;
		next_rail = static_cast<uint16_t>((next_rail + 1) % gin_ep.get_num_rails());
		flush_tail(rail_id, /*fi_more=*/true);
		flush_other_tails(rail_id);
		requests_since_doorbell = 0;
	}
