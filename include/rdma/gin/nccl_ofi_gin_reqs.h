...
public:
	...
	/* Aggregate hard-error status for this umbrella. Starts at 0 (success)
	   and is set to a negative errno by the tail-flush executor if a write
	   this umbrella OWNS hits a hard post error. A retained tail keeps a
	   back-pointer to the umbrella that created it, so a tail posted by a
	   later umbrella still records its failure here, on the original owner.
	   test() returns this once every pending subrequest has completed, so the
	   error is surfaced against the op it belongs to, never against whichever
...
public:
	...
	/* Set the fi_writemsg flags on a NEVER-POSTED request. The
	   one-tail-per-rail doorbell policy may retain a request unposted as a
	   rail's tail and only later assign its final FI_MORE / no-FI_MORE
	   decision immediately before its single post(). A request is posted
	   exactly once, so mutating flags here does not change any already-posted
	   WQE. Do not call after post(). */
	void set_flags(uint64_t new_flags) { flags = new_flags; }
